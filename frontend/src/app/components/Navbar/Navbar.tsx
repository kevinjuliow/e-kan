...
const Navbar = () => {
  ...
  return (
    <nav className={`${isBlur && 'backdrop-blur-md bg-opacity-60'} w-full flex border-gray-200 h-24 items-center fixed z-50`}>
      <div className={`max-w-screen-xl w-full flex items-center ${!isSignupOrLoginPage ? 'justify-between' : 'justify-center'} mx-auto px-8 lg:px-0`}>
        <Link href={'/'} className="flex items-center space-x-3 rtl:space-x-reverse">
          <span className={`self-center text-2xl font-semibold whitespace-nowrap transition duration-300 ${(isLanding || isDashboard) && isScrolled ? 'text-black' : (isLanding || isDashboard) && !isScrolled ? 'text-white' : (isLoginPage || isSignupPage) ? 'text-black' : 'text-black'}`}>e-Kan</span>
        </Link>

        {!isLoggedIn && !isSignupOrLoginPage && (
          <div className="flex">
            <div className="flex text-center items-center">
              <Link href={"/auth/signup"} className={`me-4 transition duration-300 font-medium ${(isLanding || isDashboard) && isScrolled ? 'text-black' : 'text-white'}`}>Sign-up</Link>
              <button onClick={handleLogin} className={`w-20 border py-1 custom-hover-button cursor-pointer rounded-md font-bold ${(isLanding || isDashboard ) && isScrolled ? 'bg-darkaqua text-white' : 'bg-white text-darkaqua'} border-none`}>Login</button>
            </div>
          </div>
        )}
