...
const PenjualProfile: React.FC<PenjualProfileProps> = ({ handleToast }) => {
  ...
  const [userData, setUserData] = useState<PenjualData>()
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.API_BASEURL}/api/penjual/profile`, {
          headers: {
  ...
  const handleFormUpdate = handleSubmit(async (values) => {
    setLoading(true)
    try {
      const response = await axios.put(`${process.env.API_BASEURL}/api/penjual/profile`, {
  ...
  return (
    <form className="w-full mb-40 relative" onSubmit={handleFormUpdate}>
      ...
      <div className="w-full md:w-[50%] flex flex-col mb-5 relative">
        <label htmlFor="alamat" className="mb-1 text-sm font-medium">Alamat</label>
        <input
          ...
          {...register("alamat")}
        />
        {errors?.alamat && <p className="absolute top-14 left-0 text-[12px] text-red-500">{errors.alamat?.message}</p>} 
      </div>
      ...
      <div className="w-full md:w-[50%] flex flex-col mb-5 relative">
        ...
        <input
          ...
          {...register("website")}
        />
        {errors?.website && <p className="absolute top-14 left-0 text-[12px] text-red-500">{errors.website?.message}</p>} 
      </div>
