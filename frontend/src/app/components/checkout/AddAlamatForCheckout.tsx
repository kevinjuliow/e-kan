...
const AddAlamatForCheckout: React.FC<AddAlamatProps> = ({ handleToast, closeFormWindow, fetchAlamat }) => {
  ...
  return (
    <div className="backdrop-blur-md bg-opacity-75 bg-white z-48 p-4 custom-popup-box-shadow rounded-lg absolute mb-20">
    <h1 className="font-bold text-base text-center mb-4">Tambah Alamat Pengiriman</h1>
    <form className="grid grid-cols-2 auto-rows-auto gap-x-2 w-full" onSubmit={handleForm}>
      ...
      <div className="col-span-2 row-start-5 justify-self-end flex items-center justify-center sm:justify-end w-full gap-x-2 sm:gap-0">
        <button onClick={closeFormWindow} className="flex w-[50%] sm:w-32 top-56 left-0 items-center justify-center bg-red-500 text-white px-4 py-2 rounded-md">
          <div className="relative right-1">
            <XMarkNonSolidIcon size={20} hexColor={"#ffffff"} />
          </div>
          <p className="ms-1 text-sm relative top-0.5">Batal</p>
        </button>
        <button type="submit" className="sm:ms-2 w-[50%] sm:w-32 top-56 right-0 md:top-12 md:right-0 flex items-center justify-center bg-mediumaqua text-white px-4 py-2 rounded-md">
          {loading ? 
...
export default AddAlamatForCheckout